(function () {
  ...
  function route(method, rawUrl, body) {
    ...
    if (url === '/api/dashboard') { const workload=Array.from({length:35},(_,i)=>{const d=new Date(Date.now()+(i-17)*86400000).toISOString().slice(0,10),list=visible.filter(t=>t.dueDate===d||t.createdAt===d||t.completedAt===d),score=list.reduce((s,t)=>s+({P0:4,P1:3,P2:2,P3:1}[t.priority]||1),0);return{date:d,taskCount:list.length,workload:score,level:Math.min(4,Math.ceil(score/2))}});return result({ tasks: visible, stats: { total: visible.length, completed: visible.filter(t => t.status === 'done').length, progress: visible.length ? Math.round(visible.reduce((s, t) => s + t.progress, 0) / visible.length) : 0, due: visible.filter(t => t.dueDate <= today && t.status !== 'done').length,points:visible.reduce((s,t)=>s+Math.round((t.points||50)*t.progress/100),0),likes:db.likes.length },workload,notes:db.notes.filter(n=>current().role==='mentor'||n.toId===current().id),likes:db.likes, users, milestones: db.milestones }) }
