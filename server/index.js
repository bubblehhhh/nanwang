...
app.post('/api/auth/send-code', (req, res) => {
  ...
  if (!phone || !/^1[3-9]\d{9}$/.test(phone)) return fail(res, 400, '请输入正确的手机号')
  const db = readDb()
  const user = db.users.find((u) => u.phone === phone)
  if (!user) return fail(res, 404, '该手机号未注册')
  const code = String(Math.floor(100000 + Math.random() * 900000))
  if (!db.resetCodes) db.resetCodes = {}
  db.resetCodes[phone] = { code, expiresAt: Date.now() + 5 * 60 * 1000, userId: user.id }
  writeDb(db)
  ok(res, { phone, demoCode: code }, `验证码已发送至手机尾号 ${phone.slice(-4)}`)
})

app.post('/api/auth/reset-password', (req, res) => {
  const { phone, code, newPassword } = req.body
  if (!phone || !code) return fail(res, 400, '手机号和验证码不能为空')
  if (typeof newPassword !== 'string' || newPassword.length < 8 || !/[^A-Za-z0-9]/.test(newPassword)) return fail(res, 400, '新密码至少8位且需包含特殊字符')
  const db = readDb()
  const record = db.resetCodes?.[phone]
  if (!record) return fail(res, 400, '请先获取验证码')
  if (record.expiresAt < Date.now()) { delete db.resetCodes[phone]; writeDb(db); return fail(res, 410, '验证码已过期，请重新获取') }
  if (record.code !== String(code)) return fail(res, 400, '验证码不正确')
  const user = db.users.find((u) => u.id === record.userId)
  if (!user) return fail(res, 404, '用户不存在')
  user.password = newPassword
  delete db.resetCodes[phone]
  writeDb(db)
  ok(res, true, '密码重置成功，请使用新密码登录')
})
