...
import { House, List, MagicStick, Trophy, Document, Sunny, ChatDotRound, SwitchButton, Aim } from '@element-plus/icons-vue'
...
const allMenus = [
  ['/', '工作台', House],
  ['/tasks', '任务中心', List],
  ['/smart-input', '智能拆解', MagicStick],
  ['/messages', '消息中心', ChatDotRound],
  ['/capabilities', '能力培养', Aim],
  ['/growth', '成长档案', Trophy],
  ['/work-library', '工作库', Document],
  ['/care', '人文关怀', Sunny]
]
...
async function checkAiKey() {
  try {
    ...
    keyDialog.value = !data.aiConfigured
  } catch {
...
async function refreshUnread() {
  ...
  try {
    const data = unwrap(await api.get('/care'))
...
async function saveAiKey() {
  ...
  try {
    await api.request({ method: replacingKey.value ? 'put' : 'post', url: '/settings/ai-key', data: { apiKey: apiKey.value.trim() } })
    apiKey.value = ''
    keyDialog.value = false
    replacingKey.value = false
    ElMessage.success('密钥已验证并保存到后端')
  } catch (error) {
...
onMounted(() => {
  checkAiKey()
  refreshUnread()
  refreshTimer = window.setInterval(refreshUnread, 20000)
  ...
  resetHandler = async (e) => {
    if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'd' && auth.loggedIn) {
      ...
      localStorage.removeItem('xinhuo_modules')
      location.reload()
    }
  }
...
onBeforeUnmount(() => {
  window.removeEventListener('xinhuo-ai-quota', showQuotaDialog)
  if (resetHandler) window.removeEventListener('keydown', resetHandler)
...
<template>
  ...
  <div v-else class="app-shell">
    <aside class="sidebar">
      ...
      <nav>
        <router-link v-for="[path, label, icon] in menus" :key="path" :to="path">
          <el-icon><component :is="icon" /></el-icon>
          <span>{{ label }}</span>
          <i v-if="path === '/messages' && unreadMessages" class="nav-badge">{{ unreadMessages > 99 ? '99+' : unreadMessages }}</i>
        </router-link>
      </nav>
      <div class="sidebar-foot">
        <div class="mini-avatar" :style="{ background: auth.user?.avatarColor || '' }" @click="router.push('/profile')" title="点击进入个人设置">{{ auth.user?.avatar }}</div>
        <div class="sidebar-foot-info" @click="router.push('/profile')" title="点击进入个人设置"><b>{{ auth.user?.name }}</b><small>{{ auth.user?.roleName }} · {{ auth.user?.position }}</small></div>
        <el-button text circle @click="logout"><el-icon><SwitchButton /></el-icon></el-button>
      </div>
    </aside>
    <main class="main-area">
      <header class="topbar">
        <div><span class="crumb">南网·薪火</span><span>/</span><b>{{ route.meta.title || menus.find(m => m[0] === route.path)?.[1] }}</b></div>
        <div class="top-actions">
          <el-tag v-if="unreadMessages" type="danger" effect="plain">新消息 {{ unreadMessages }}</el-tag>
          <el-tag v-else type="success" effect="plain">系统运行正常</el-tag>
...
<style scoped>
.nav-badge{margin-left:auto;min-width:20px;height:20px;padding:0 6px;border-radius:999px;background:#ff5b5b;color:#fff;font-style:normal;font-size:11px;display:grid;place-items:center}
@media(max-width:620px){.nav-badge{position:absolute;right:8px;top:4px;min-width:16px;height:16px;padding:0 4px;font-size:10px}}
</style>
