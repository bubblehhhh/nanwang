...
async function submit() {
  ...
  if (!form.username.trim()) return ElMessage.warning('请输入姓名或工号')
  if (!form.password) return ElMessage.warning('请输入登录密码')
  if (form.password.length < 8 || !/[^A-Za-z0-9]/.test(form.password)) return ElMessage.warning('密码需至少8位且包含特殊字符')
  loading.value = true
  try {
    const data = unwrap(await api.post('/auth/login', form)); auth.setSession(data)
    ElMessage.success(data.user.role === 'mentor' ? `欢迎回来，${data.user.name}。您正在带教张三、赵六。` : `欢迎回来，${data.user.name}。您的师父是${data.user.mentor}。`)
    router.push('/')
    ...
    <section class="login-panel">
      <div class="login-box">
        ...
        <el-form @submit.prevent="submit" label-position="top">
          <el-form-item label="姓名或工号"><el-input v-model="form.username" size="large" :prefix-icon="User" placeholder="请输入姓名或工号" @keyup.enter="submit" /></el-form-item>
