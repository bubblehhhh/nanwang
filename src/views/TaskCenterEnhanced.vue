...
const selectedVerifyIds=computed(()=>scopedTasks.value.filter(task=>task.status==='verify'&&task.checked).map(task=>task.id))
const projectGroups=computed(()=>{
  const map={}
  scopedTasks.value.forEach(t=>{
    const pn=t.projectName||'未分类'
    if(!map[pn])map[pn]={projectName:pn,tasks:[],sources:{}}
    map[pn].tasks.push(t)
    const src=t.source||'导师任务'
    const srcKey=/会议/.test(src)?'meeting':'manual'
    if(!map[pn].sources[srcKey])map[pn].sources[srcKey]={label:srcKey==='meeting'?'会议纪要发布':'主动发布',tasks:[]}
    map[pn].sources[srcKey].tasks.push(t)
  })
...
async function bulkVerify(){if(!selectedVerifyIds.value.length)return ElMessage.warning('请先勾选待核销任务');try{const{value}=await ElMessageBox.prompt('请输入本次统一核销意见','一键核销',{inputValue:'符合完成标准，同意核销。',confirmButtonText:'确认核销'});bulkVerifying.value=true;await api.post('/tasks/verify-bulk',{taskIds:selectedVerifyIds.value,comment:value});ElMessage.success(`已批量核销 ${selectedVerifyIds.value.length} 项任务`);load()}catch(e){if(e!=='cancel'&&e!=='close')ElMessage.error(errorText(e))}finally{bulkVerifying.value=false}}
...
<el-tag :type="src.label==='会议纪要发布'?'warning':'primary'" effect="plain" size="small">{{src.label}}</el-tag>
<span>{{src.tasks.length}} 项</span>
</div>
<div class="project-task-list">
<div v-for="task in src.tasks" :key="task.id" class="project-task-row">
<el-checkbox v-if="auth.user?.role==='mentor'&&task.status==='verify'" v-model="task.checked"/>
<el-tag size="small" :type="task.priority==='P0'?'danger':task.priority==='P1'?'warning':'primary'">{{task.priority}}</el-tag>
...
<el-empty v-if="!projectGroups.length" description="暂无项目任务"/>
</div>
...
@media(max-width:1000px){.office-kanban{grid-template-columns:repeat(4,240px)}.project-task-row{flex-wrap:wrap}.apprentice-brief{grid-template-columns:1fr}.bulk-verify-bar{flex-direction:column;align-items:flex-start}}
</style>
