...
import { Plus,Search,EditPen,CircleCheck } from '@element-plus/icons-vue'
import api,{errorText,unwrap} from '../api'
import { useAuthStore } from '../store'

const auth=useAuthStore();const tasks=ref([]);const users=ref([]);const skills=ref([]);const filter=ref('all');const keyword=ref('');const dialog=ref(false);const progressDialog=ref(false);const active=ref(null);const apprenticeFilter=ref('all');const openPriorities=ref([])
...
async function load(){tasks.value=unwrap(await api.get('/tasks'));users.value=unwrap(await api.get('/dashboard')).users.filter(u=>u.role==='apprentice');skills.value=unwrap(await api.get('/capabilities',{params:{userId:users.value[0]?.id||1}})).skills}
...
<b>{{task.title}}</b>
<span>{{task.assignee}} · {{task.startTime||'时间待定'}}-{{task.endTime||''}} · {{task.projectName}} · 截止 {{task.dueDate}}</span>
