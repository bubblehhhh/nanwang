...
const statuses={todo:'未完成',doing:'未完成',verify:'未完成',done:'完成'}
...
const filtered=computed(()=>tasks.value.filter(t=>(filter.value==='all'||(filter.value==='done'?t.status==='done':t.status!=='done'))&&(!keyword.value||`${t.title}${t.assignee}${t.projectName}`.includes(keyword.value))))
...
const kanban=computed(()=>[{status:'pending',title:'未完成',tasks:scopedTasks.value.filter(t=>t.status!=='done')},{status:'done',title:'完成',tasks:scopedTasks.value.filter(t=>t.status==='done')}])
...
<el-segmented v-model="filter" :options="[{label:'全部',value:'all'},{label:'未完成',value:'pending'},{label:'完成',value:'done'}]"/>
...
<style scoped>
.office-kanban{display:grid;grid-template-columns:repeat(2,1fr);gap:14px;align-items:start;overflow:auto}.office-kanban>section{background:#f4f6f8;border:1px solid #e1e6eb;min-height:420px;padding:12px}.office-kanban>section>header{display:flex;justify-content:space-between;padding:4px 3px 11px}.office-kanban>section>header span{background:#dfe5ea;min-width:22px;text-align:center;border-radius:3px}.kanban-stack{display:flex;flex-direction:column;gap:8px}.kanban-stack article{background:#fff;border:1px solid #e0e5ea;border-top:3px solid #8693a5;padding:11px}.office-kanban .pending article{border-top-color:#2b77c0}.office-kanban .done article{border-top-color:#087c66}.kanban-top,article footer{display:flex;justify-content:space-between;align-items:center}.kanban-top strong{font-size:10px;color:#087c66}.kanban-stack h3{font-size:13px;line-height:1.45;margin:9px 0 5px}.kanban-stack p,.kanban-stack small{font-size:10px;color:#7f8b9a}.kanban-stack footer{margin-top:8px}.toolbar{flex-wrap:wrap}.toolbar>.el-segmented:last-child{margin-left:auto}
...
@media(max-width:1000px){.office-kanban{grid-template-columns:1fr}.project-task-row{flex-wrap:wrap}}
</style>
